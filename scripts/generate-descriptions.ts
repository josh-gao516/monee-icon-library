// ─────────────────────────────────────────────────────────────────────────
// 决策日志 · 为什么这里是 text-only,没有 vision(看图)链路
// ─────────────────────────────────────────────────────────────────────────
// 2026-06 试过给本脚本加 vision 增强(渲染 raw/<key>.svg → PNG → 喂给模型看图,
// 用 @resvg/resvg-js 渲染、走 SDK image block)。对 user.2.fill 做了 text↔vision
// 并排对比,结论是 vision 对本库这类 24×24 纯黑剪影图标【负收益】:
//   · 渲染本身清晰正确(肉眼确认是"两个人")。
//   · 但 Haiku 看图把"第二个人 + 小箭头"误读成"一个人 + 喇叭/扬声器",
//     description 跑偏到"营销推广",tags 还混入图里根本不存在的 `megaphone`。
//   · 反观 text 链路:key 名 `user.2.fill` 已编码语义(两个 user),输出更准。
// 即:对高度浓缩的剪影图标,key 名里的语义比像素更可靠。故放弃 vision,
// 卸载 @resvg/resvg-js,只保留 text-only。若日后图标更复杂/写实,可重新评估。
// ─────────────────────────────────────────────────────────────────────────
